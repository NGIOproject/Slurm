...
env_vars_t env_vars[] = {
  ...
  {"SLURM_FILESYSTEM_MOUNT", OPT_STRING,   &sbopt.filesystem_mountpoint,  NULL },
...
static bool _opt_verify(void)
{
	...
	_fullpath(&opt.map_json, opt.cwd);	// NEXTGenIO
	...
	if (opt.nvram_options) {
		...
		if (! xstrstr(conf->node_features_plugins, "nvram_")) {
			error("NVRAM options can be used with NVRAM features only.");
			slurm_conf_unlock();
			exit(error_exit);
		}
		slurm_conf_unlock();
	...
	conf = slurm_conf_lock();
	if (xstrstr(conf->node_features_plugins, "nvram_") && !(opt.nvram_options) ) {
		error("NVRAM options must be used with NVRAM features.");
		slurm_conf_unlock();
		exit(error_exit);
	}
	slurm_conf_unlock();
