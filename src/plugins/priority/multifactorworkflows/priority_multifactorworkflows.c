/*****************************************************************************\
 *  priority_multifactorworkflows.c - slurm multifactor priority plugin.
 *****************************************************************************
 *
 ...
 */
const char plugin_name[]	= "Priority MULTIFACTORWORKFLOWS plugin";
const char plugin_type[]	= "priority/multifactorworkflows";
...
static void *_decay_thread(void *no_data)
{
	...
	while (!plugin_shutdown) {
		...
		if (_apply_decay(real_decay) != SLURM_SUCCESS) {
			error("priority/multifactorworkflows: problem applying decay");
			running_decay = 0;
	...
	if (xstrcasecmp(temp, "accounting_storage/slurmdbd")
		...
		if (!cluster_cpus)
			fatal("We need to have a cluster cpu count "
			      "before we can init the priority/multifactorworkflows "
			      "plugin");
		...
		if (weight_fs) {
			fatal("It appears you don't have any association "
			      ...
			      "The priority/multifactorworkflows plugin requires "
...
extern uint32_t priority_p_set(uint32_t last_prio, struct job_record *job_ptr)
{
	if (priority_debug)
		if (job_list && list_count(job_list)) {
			int count = list_count(job_list);
			debug2("%s: job_list count:%d", __func__, count);
			ListIterator itr2;
	...
	debug2("%s: initial priority for job %u is %u", __func__, job_ptr->job_id, priority);
