/*****************************************************************************\
 *  slurm_acct_gather_nvram.c - implementation-independent job nvram
...
typedef struct slurm_acct_gather_nvram_ops {
...
} slurm_acct_gather_nvram_ops_t;
/*
 * These strings must be kept in the same order as the fields
 * declared for slurm_acct_gather_fnvram_ops_t.
 */
static const char *syms[] = {
	"acct_gather_nvram_p_node_update",
	"acct_gather_nvram_p_conf_options",
	"acct_gather_nvram_p_conf_set",
	"acct_gather_nvram_p_conf_values",
	"acct_gather_nvram_p_get_data",
};

static slurm_acct_gather_nvram_ops_t ops;
...
extern int acct_gather_nvram_init(void)
{
	int retval = SLURM_SUCCESS;
	char *plugin_type = "acct_gather_nvram";
	char *type = NULL;
	...
	type = slurm_get_acct_gather_nvram_type();
...
extern int acct_gather_nvram_fini(void)
{
 ...
 */
extern int acct_gather_nvram_g_get_data(acct_gather_data_t *data)
{
	int retval = SLURM_SUCCESS;

	if (acct_gather_nvram_init() < 0)
		return SLURM_ERROR;
	retval = (*(ops.get_data))(data);
	return retval;
}

extern int acct_gather_nvram_startpoll(uint32_t frequency)
{
	int retval = SLURM_SUCCESS;
	...
	if (!acct_shutdown) {
		error("acct_gather_nvram_startpoll: "
	...
	if (frequency == 0) {   /* don't want dynamic monitoring? */
		debug2("acct_gather_nvram dynamic logging disabled");
		return retval;
	}
	...
	debug3("acct_gather_nvram dynamic logging enabled");

	return retval;
}


extern int acct_gather_nvram_g_conf_options(s_p_options_t **full_options,
						  int *full_options_cnt)
{
        if (acct_gather_nvram_init() < 0)
		return SLURM_ERROR;
...
extern int acct_gather_nvram_g_conf_set(s_p_hashtbl_t *tbl)
{
        if (acct_gather_nvram_init() < 0)
		return SLURM_ERROR;
...
extern int acct_gather_nvram_g_conf_values(void *data)
{
	if (acct_gather_nvram_init() < 0)
		return SLURM_ERROR;
